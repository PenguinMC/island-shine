...
//   1) Admin notification with the full quote details (to the admin recipients)
//   2) Customer confirmation ("We got your quote...") to the address they entered
//
// Env vars:
//   islandshine / RESEND_API_KEY  (required) - the Resend API key
//   TO_EMAILS  (optional) - comma-separated admin recipients
//   FROM_EMAIL (optional) - verified Resend sender (e.g. "Island Shine <quotes@islandshines.com>")
...
module.exports = async (req, res) => {
  ...
  <div style="max-width:560px;margin:0 auto;font-family:-apple-system,Segoe UI,Roboto,sans-serif;border:1px solid #d3e2ee;border-radius:14px;overflow:hidden">
    ...
    ${inner}
    <div style="background:#eef6fc;padding:12px 24px;font:12px/1.4 monospace;color:#46627a">islandshines.com &nbsp;·&nbsp; (843) 730-3717</div>
  </div>`;
  ...
  try {
    const adminRes = await send(admins, `New quote request${name ? ' from ' + name : ''}`, adminHtml, email || undefined);
    if (!adminRes.ok) { res.status(502).json({ error: 'Admin send failed', detail: await adminRes.text() }); return; }

    // Customer confirmation is best-effort (won't fail the request if it bounces)
    if (email) {
      try { await send(email, 'We got your quote — Island Shine', customerHtml, 'hello@islandshines.com'); } catch (e) {}
    }
